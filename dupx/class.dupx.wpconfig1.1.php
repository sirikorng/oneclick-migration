...
class DUPX_WPConfig
{
	 ...
	 *  @return null
	 */
    public static function updateStandard($database_connection_config)
	{
		...
		$db_host = ($database_connection_config->getDbport() == 3306) ? $database_connection_config->getDbhost() : "{$database_connection_config->getDbhost()}:{$database_connection_config->getDbport()}";

		$replace = array(
		    "'DB_NAME', ".'\''.$database_connection_config->getDbname().'\'',
		    "'DB_USER', ".'\''.$database_connection_config->getDbuser().'\'',
		    "'DB_PASSWORD', ".'\''.DUPX_U::pregReplacementQuote($database_connection_config->getDbpass()).'\'',
		    "'DB_HOST', ".'\''.$database_connection_config->getDbhost().'\'');
	 ...
	 *  @return null
	 */
	public static function updateExtended($web_config)
	{
		$config_file = '';
		$root_path		= DUPX_U::setSafePath($GLOBALS['CURRENT_ROOT_PATH']);
		$wpconfig_path	= "{$root_path}/wp-config.php";
		...
		$patterns	 = array(
			...
			"/('|\")WP_SITEURL.*?\)\s*;/");
		$replace	 = array(
			"'WP_HOME', '{$web_config->getUrl_new()}');",
			"'WP_SITEURL', '{$web_config->getUrl_new()}');");
		...
		if (isset($defines['WP_CONTENT_DIR'])) {
		    $val = str_replace($web_config->getPath_old(), $web_config->getPath_new(), DUPX_U::setSafePath($defines['WP_CONTENT_DIR']), $count);
			if ($count > 0) {
		...
		if (isset($defines['WP_CONTENT_URL'])) {
		    $val = str_replace($web_config->getUrl_old() . '/', $web_config->getUrl_new() . '/', $defines['WP_CONTENT_URL'], $count);
			if ($count > 0) {
		...
		if (isset($defines['WP_TEMP_DIR'])) {
		    $val = str_replace($web_config->getPath_old(), $web_config->getPathl_new(), DUPX_U::setSafePath($defines['WP_TEMP_DIR']) , $count);
			if ($count > 0) {
		...
		if (isset($defines['DOMAIN_CURRENT_SITE'])) {
		    $mu_newDomainHost = parse_url($web_config->getUrl_new(), PHP_URL_HOST);
		...
		if (isset($defines['PATH_CURRENT_SITE'])) {
		    $mu_newUrlPath = parse_url($web_config->getUrl_new(), PHP_URL_PATH);
